...
export const getBasketState = (state: State): { productsInBasket: Record<number, number>; discont: number; submittedCoupon: Coupon; hasOrderError: boolean; productCards: ProductCard[] } => ({ productsInBasket: state[NameSpace.BasketData].productsInBasket, discont: state[NameSpace.BasketData].discont, submittedCoupon: state[NameSpace.BasketData].submittedCoupon, hasOrderError: state[NameSpace.BasketData].hasOrderError, productCards: state[NameSpace.CatalogData].productCards });
